...
export default function Layout() {
  ...
  const { logout: keycloakLogout, isAuthenticated: isKeycloakAuthenticated, userInfo: keycloakUserInfo } = useKeycloakAuth()
  const { isDark, toggleTheme } = useTheme()

  // Get user display info from either auth system
  const displayName = keycloakUserInfo?.name || user?.name || 'User'
  const displayEmail = keycloakUserInfo?.email || user?.email
                    ...
                    <div style={{ borderTop: isDark ? '1px solid var(--surface-500)' : '1px solid #e5e5e5', paddingTop: '4px' }}>
                      <button
                        onClick={() => {
                          ...
                          console.log('[LOGOUT] isKeycloakAuthenticated:', isKeycloakAuthenticated)
                          setUserMenuOpen(false)

                          // Logout from both auth systems
                          console.log('[LOGOUT] Calling legacyLogout()')
                          legacyLogout() // Clear localStorage (legacy auth)

                          if (isKeycloakAuthenticated) {
                            console.log('[LOGOUT] Calling keycloakLogout()')
                            keycloakLogout() // Clear sessionStorage + redirect to Keycloak logout
                          } else {
                            console.log('[LOGOUT] Not authenticated via Keycloak, redirecting to /login')
