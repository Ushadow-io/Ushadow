...
interface ServiceOption {
  ...
  description?: string
  default?: any
  choices?: { value: string; label: string; requires_env?: string }[]
  min?: number
  max?: number
  step?: number
}
...
interface ServiceInstance {
  ...
  name: string
  description: string
  type: string  // Was 'template' - now just the type (memory, llm, etc.)
  mode: 'cloud' | 'local' | null
  is_default: boolean
  enabled: boolean
  configured: boolean  // Backend-computed: all required config present
  missing_config: string[]  // Backend-computed: env vars that need setup
  container_status?: string  // Backend-computed: running, stopped, not_found
  options: Record<string, ServiceOption>  // New API format
  required_env: string[]  // New field
  tags: string[]
  containers?: string[]  // For local services
  api_base?: string  // For cloud services
  // Computed field added in loadData for backwards compatibility
  config_schema?: any[]
}
...
export default function ServicesPage() {
  ...
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set(['memory', 'llm', 'transcription', 'conversation_engine']))
  ...
  // Uses lightweight /status endpoint instead of full /installed
  useEffect(() => {
    const pollStatus = async () => {
      try {
        ...
        // Update container_status for each service
        setServiceInstances(prev => prev.map(service => {
          const newStatus = statusMap[service.service_id]
          if (newStatus !== undefined && newStatus !== service.container_status) {
            // Clear starting spinner if service is now running
            if (startingService === service.service_id && newStatus === 'running') {
              setStartingService(null)
              setMessage({ type: 'success', text: 'Service is now running' })
            }
            return { ...service, container_status: newStatus }
          }
          return service
        }))
      } catch (error) {
  ...
  const loadData = async (showLoading = true) => {
    try {
      ...
      // Backend now provides configured/missing_config - no need for separate config call
      const instancesResponse = await servicesApi.getInstalled()
      const instances = instancesResponse.data

      // Transform instances to add config_schema from options + required_env
      const transformedInstances = instances.map((service: any) => {
        // Start with options converted to config_schema format
        const optionsSchema = service.options
          ? Object.entries(service.options).map(([key, opt]: [string, any]) => ({
              key,
              label: opt.label,
              type: opt.type,
              required: false,
        ...
        // Add required_env vars as config fields (marked as global)
        const envSchema = (service.required_env || []).map((envVar: string) => ({
          key: envVar.toLowerCase(),
          label: envVar.replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()),
          type: 'password',
          required: true,
          description: `Required API key (configured globally)`,
          isGlobal: true,
          envVar: envVar,
        }))
        ...
        return {
          ...service,
          config_schema: [...envSchema, ...optionsSchema]
        }
      })

      // Build config for service-specific options (non-global fields)
      // Global field status comes from service.configured / service.missing_config
      const effectiveConfigs: any = {}
      transformedInstances.forEach((service: any) => {
        effectiveConfigs[service.service_id] = {}
        service.config_schema?.forEach((field: any) => {
          if (!field.isGlobal && field.default !== undefined) {
            effectiveConfigs[service.service_id][field.key] = field.default
          }
        })
      })

      setServiceInstances(transformedInstances)
      setServiceConfigs(effectiveConfigs)

      // Clear starting spinner if the service we were starting is now running
      if (startingService) {
        const startedService = transformedInstances.find(
          (s: any) => s.service_id === startingService && s.container_status === 'running'
        )
        if (startedService) {
          setStartingService(null)
          setMessage({ type: 'success', text: 'Service is now running' })
  ...
  const handleStartService = async (serviceId: string) => {
    ...
    try {
      const response = await dockerApi.startService(serviceId)
      // Check if the operation actually succeeded
      if (response.data?.success === false) {
        // Show error on the service card
        setServiceErrors(prev => ({ ...prev, [serviceId]: response.data?.message || 'Failed to start service' }))
        setStartingService(null)
      } else {
        // Status polling will detect when container is running
      }
    } catch (error: any) {
  ...
  const confirmStopService = async () => {
    ...
    try {
      const response = await dockerApi.stopService(serviceId)
      // Check if the operation actually succeeded
      if (response.data?.success === false) {
        setServiceErrors(prev => ({ ...prev, [serviceId]: response.data?.message || 'Failed to stop service' }))
      } else {
        // Clear any error and refresh status
        setServiceErrors(prev => {
          const next = { ...prev }
          delete next[serviceId]
          return next
      ...
      const errorMsg = error.response?.data?.detail || error.response?.data?.message || 'Failed to stop service'
      setServiceErrors(prev => ({ ...prev, [serviceId]: errorMsg }))
    }
  }
  ...
  const getServiceStatus = (service: ServiceInstance) => {
    // Use backend-computed configured status
    const isConfigured = service.configured

    // Rule 2: Not configured services - warning with alert icon
    if (!isConfigured) {
      return {
    ...
    // Rule 4: Local services - check container status from API response
    if (service.container_status === 'running') {
      // Container running - show Stop button
      return {
        state: 'running',
        label: 'Running',
        color: 'success',
    ...
    // All other states (not_found, exited, stopped, dead, created) - show Start button
    return {
      state: 'stopped',
      label: 'Start',
      color: 'neutral',
  ...
  // Group services by type (was category)
  const servicesByCategory = serviceInstances.reduce((acc, service) => {
    const category = service.type  // Now just the type directly
    if (!acc[category]) {
      acc[category] = []
    }
  ...
  const categories = [
    ...
    { id: 'conversation_engine', name: 'Conversation', description: 'Audio processing with AI analysis' },
  ]
                      ...
                      >
                        <div className="p-4">
                          ...
                          {!isEditing && (() => {
                            ...
                            // mode === null or mode === 'local' means it's a local service
                            const isLocalService = service.mode !== 'cloud'
                            const isClickable = service.enabled && isLocalService && (status.canStart || status.canStop)
                        ...
                        {/* Service Error - inline with the card */}
                        {serviceErrors[service.service_id] && (
                          <div
                            className="mt-2 p-2 bg-error-50 dark:bg-error-900/30 border border-error-200 dark:border-error-800 rounded-lg"
                            data-testid={`service-error-${service.service_id}`}
                          >
                            ...
                            <button
                              ...
                              className="mt-1 text-xs text-error-600 dark:text-error-400 underline hover:no-underline"
                            >
                              Dismiss
                            </button>
                          </div>
                        ...
                        {service.config_schema && service.config_schema.length > 0 && (
                          <>
                            {/* Config Fields - Show when expanded or editing */}
                              ...
                              <div className={`space-y-2 px-4 pb-4 pt-3 border-t border-neutral-200 dark:border-neutral-700 ${isEditing ? 'mt-0' : 'mt-0'}`}>
                                {service.config_schema
                                  .filter((f: any) => {
                                    ...
                                    // In view mode, show global fields (API keys) always
                                    if (f.isGlobal) return true
                                    // Show non-global fields only if they have a value
                                      ...
                                      {isEditing ? (
                                        <>
                                          <label
                                            ...
                                            className="text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1 flex items-center gap-2"
                                          >
                                            {field.label}
                                          ...
                                          <div className="text-xs">
                                            {field.isGlobal ? (
                                              <div className="flex items-center gap-2">
                                                {service.missing_config?.includes(field.envVar) ? (
                                                  <span className="text-error-600 dark:text-error-400">Not configured</span>
                                                ) : (
                                        ...
                                        <>
                                          <span className="text-xs text-neutral-500 dark:text-neutral-400 flex-shrink-0 flex items-center gap-1">
                                            {field.label}:
                                            {field.isGlobal && (
                                              <span className="px-1 py-0.5 text-[9px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded">
                                                Global
                                              </span>
                                            )}
                                          </span>
                                          <div className="text-xs flex-1 truncate">
                                            {field.isGlobal ? (
                                              // Use missing_config from backend to determine if configured
                                              service.missing_config?.includes(field.envVar) ? (
                                                <span className="text-error-600 dark:text-error-400">Not set</span>
                                              ) : (
                                                <span className="text-success-600 dark:text-success-400">••••••••</span>
                                              )
                                            ) : (
                        ...
                        {/* Enable/Disable Toggle - Bottom Right */}
                        {!isEditing && (
                          <div className="px-4 pb-3 flex justify-end">
                            <button
                              ...
                              disabled={togglingEnabled === service.service_id || !service.configured}
                              ...
                              title={!service.configured ? 'Configure service first' : (service.enabled ? 'Click to disable' : 'Click to enable')}
                            >
