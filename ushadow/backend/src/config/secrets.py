...
def get_keycloak_client_secret() -> str:
    ...
    Priority:
    1. secrets.yaml (keycloak.backend_client_secret) - persisted by setup script
    2. Environment variable KEYCLOAK_BACKEND_CLIENT_SECRET - fallback
    ...
    # Try reading directly from secrets.yaml
    ...
    if secrets_path.exists():
        try:
            with open(secrets_path, 'r') as f:
                ...
                secret = keycloak_secrets.get('backend_client_secret')
                if secret:
                    return secret
        except Exception as e:
    ...
    secret = os.environ.get("KEYCLOAK_BACKEND_CLIENT_SECRET")
    if secret:
        return secret

    raise ValueError(
        "KEYCLOAK_BACKEND_CLIENT_SECRET not found. "
        "Should be set in config/secrets.yaml by setup script."
    )
