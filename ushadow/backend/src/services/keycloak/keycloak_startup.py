...
def get_current_redirect_uris() -> List[str]:
    ...
    # USHADOW_PUBLIC_URL (K8s / Tailscale ingress public URL)
    public_url = os.getenv("USHADOW_PUBLIC_URL")
    if public_url and public_url.startswith("https://"):
        redirect_uris.append(f"{public_url.rstrip('/')}/oauth/callback")
        logger.info(f"[KC-STARTUP] 📡 Adding USHADOW_PUBLIC_URL URI: {public_url}")
...
async def register_url_with_keycloak(public_url: str) -> bool:
    """
    Register a specific public URL's redirect URIs with Keycloak.

    Called after Tailscale ingress is configured so the new hostname is
    immediately usable for OAuth without a backend restart.
    """
    ...
    async with _registration_lock:
        try:
            ...
            if success:
                await admin_client.update_post_logout_redirect_uris(
                    client_id="ushadow-frontend",
                    post_logout_redirect_uris=[base, base + "/"],
                    merge=True,
                )
                logger.info(f"[KC-STARTUP] ✅ Registered KC URIs for {public_url}")
            return success
        except Exception as e:
            logger.warning(f"[KC-STARTUP] ⚠️  Failed to register KC URIs for {public_url}: {e}")
            return False
