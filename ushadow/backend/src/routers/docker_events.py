...
@router.get("/dockerstatus_events")
async def docker_status_events_stream() -> EventSourceResponse:
    """
    ...
    async def event_generator():
        ...
        logger.info("SSE client connected for Docker status events")

        # Send initial heartbeat immediately (before blocking on Docker events)
        yield {
            "event": "connected",
            "data": json.dumps({
