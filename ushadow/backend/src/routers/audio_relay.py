...
async def audio_relay_websocket(
    ...
    try:
        ...
        bridged_token = await bridge_to_service_token(token, audiences=["ushadow", "chronicle"])
        if bridged_token:
            if bridged_token != token:
                logger.info(f"[AudioRelay] ✓ Bridged Keycloak token to service token")
            token = bridged_token  # Use bridged token for destinations
        else:
            logger.error(f"[AudioRelay] Failed to validate or bridge token")
